...
int parseDateTimeTZD(IteratorT& it, const IteratorT& end);

template<typename IteratorT>
int parseDateTimeMonth(IteratorT& it, const IteratorT& end);

template<typename IteratorT>
int parseDateTimeDayOfWeek(IteratorT& it, const IteratorT& end);

template<typename IteratorT>
int parseDateTimeAMPM(IteratorT& it, const IteratorT& end, int hour);
...
template<typename IteratorT>
char parseDateTime(const std::string& fmt, IteratorT it, IteratorT end , DateTime& dateTime, int& timeZoneDifferential)
{
	int year   = 0;
	int month  = 0;
	int day    = 0;
	...
	while (itf != endf && it != end)
	{
		if (*itf == '%')
		{
			if (++itf != endf)
			{
				...
				case 'b':
				case 'B':
					month = parseDateTimeMonth(it, end);
					break;
				case 'd':
				case 'e':
				case 'f':
				...
				case 'a':
				case 'A':
					hour = parseDateTimeAMPM(it, end, hour);
					break;
				case 'M':
					SKIP_JUNK();
...
{
	parseDateTime(fmt, str.begin(), str.end(), dateTime, timeZoneDifferential);
}
...
{
	return parseDateTimeMonth(it,end);
}
...
{
	return parseDateTimeDayOfWeek(it,end);
}
...
DateTime DateTimeParser::parseFromStream(const std::string& fmt, std::istream & is)
{
	int timeZoneDifferential = 0;
	DateTime dateTime;
	parseFromStream(fmt, is, dateTime, timeZoneDifferential);	
	dateTime.makeUTC(timeZoneDifferential);
	return dateTime;
}

DateTime DateTimeParser::parseFromStream(const std::string& fmt, std::istream & is, int & timeZoneDifferential)
{
	DateTime dateTime;
	parseFromStream(fmt, is, dateTime, timeZoneDifferential);
	return dateTime;
...
void DateTimeParser::parseFromStream(const std::string& fmt, std::istream & is, DateTime & dateTime)
{
	dateTime = parseFromStream(fmt, is);
}

void DateTimeParser::parseFromStream(const std::string& fmt, std::istream & is, DateTime & dateTime, int & timeZoneDifferential)
{
	// A good thing here would be to use a scoped flag guard for the stream
	// but it should work as is

	// Get the current flags
	std::ios::fmtflags oldf = is.flags();

	try
	{
		// unset skipws
		is.unsetf(std::ios::skipws);
		char last = parseDateTime
		(
			fmt, 
		...
		// \xff is used a marker that we reached the end
		if(!is.eof() && last != '\xff')
		{
			is.putback(last);
		}

		// Restore to the flags the stream had before we got it
		is.flags(oldf);
	}
	catch (...)
	{
		// Exception caught no matter which restore the stream flags 
		// for consistency
		is.flags(oldf);
		// then rethrow 
		throw;
...
bool DateTimeParser::tryParseFromStream(const std::string& fmt, std::istream & is, DateTime & dateTime)
{
	try
	{ 
		parseFromStream(fmt, is, dateTime); 
	}
	catch(Exception&)
	{
		return false;
...
bool DateTimeParser::tryParseFromStream(const std::string& fmt, std::istream & is, DateTime & dateTime, int & timeZoneDifferential)
{
	try
	{ 
		parseFromStream(fmt, is, dateTime, timeZoneDifferential); 
